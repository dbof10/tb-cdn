...
const FINNHUB_API_URL =
    "https://finnhub.io/api/v1/stock/symbol?exchange=US&token=cv4odf1r01qn2gabautgcv4odf1r01qn2gabauu0";
...
async function fetchStocks() {
    try {
        ...
        return response.data.filter(
            (stock) => stock.type === "Common Stock" && stock.mic !== "OOTC"
        ); // Exclude OTC stocks
    } catch (error) {
    ...
    return new Promise((resolve) => {
        https
            .get(logoUrl, (response) => {
                ...
                fileStream.on("error", () => {
                    fileStream.close();
                    if (fs.existsSync(filePath)) fs.unlinkSync(filePath); // Delete incomplete file
                    resolve(symbol);
                });
            })
            .on("error", () => {
